...
export function CollabControls() {
  ...
  return (
    <div className="inline-flex h-9 items-center gap-2 rounded border border-neutral-300 bg-white px-2 dark:border-neutral-700 dark:bg-neutral-800">
      <StatusDot status={status} />
      <PeerAvatars peers={peers} />
      {otherPeers.length === 0 ? (
        <span className="text-xs text-neutral-500 dark:text-neutral-400">Waiting for others…</span>
      ) : null}
      <button
        type="button"
        ...
        className="inline-flex h-7 items-center gap-1 rounded border border-neutral-300 bg-white px-1.5 text-xs text-neutral-800 hover:bg-neutral-50 dark:border-neutral-600 dark:bg-neutral-700 dark:text-neutral-200 dark:hover:bg-neutral-600"
      >
      ...
      <button
        ...
        className="inline-flex h-7 w-7 items-center justify-center rounded border border-neutral-300 bg-white text-neutral-600 hover:bg-neutral-50 dark:border-neutral-600 dark:bg-neutral-700 dark:text-neutral-300 dark:hover:bg-neutral-600"
      >
    ...
    <div className="flex -space-x-1.5">
      {peers.map((peer) => (
        <span
          ...
          className="inline-flex h-6 w-6 items-center justify-center rounded-full border border-white text-[10px] font-bold uppercase text-white shadow dark:border-neutral-900"
        >
          {peer.name.slice(0, 2)}
        </span>
      ))}
